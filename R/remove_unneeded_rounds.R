...
#'
#' @param x dataframe with columns called both "Round_X" and "Round_X_Results"
#'   where X is a number
#' @return returns a dataframe where Round_X columns that do not have a
...
remove_unneeded_rounds <- function(x) {
  ...
  attempt_cols <-
    stringr::str_remove(stringr::str_subset(names(x), "^Round_"), "_Attempts?_?\\d{0,}")
